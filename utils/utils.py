...
from .logger.error_levels import Level
...
def return_embed(
    ctx: discord_slash.SlashContext, name, content, color: discord.Color
) -> discord.Embed:
...
def run_checks():
    ...
    if CONFIG.LOG_LEVEL == "DEBUG":
        error_level = Level.DebugLevel()
    elif CONFIG.LOG_LEVEL == "INFO":
        error_level = Level.InfoLevel()
    elif CONFIG.LOG_LEVEL == "WARNING":
        error_level = Level.WarningLevel()
    elif CONFIG.LOG_LEVEL == "ERROR":
        error_level = Level.ErrorLevel()
    else:
        print("Invalid log error level defined. Defaulting to INFO")
        error_level = Level.InfoLevel()

    LOGGER = logger.Logger(
        should_log_to_file=True, only_print_over_and_including_severity=error_level
    )
