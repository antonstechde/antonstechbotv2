...
class PrivateChannel(Cog):
    ...
    @cog_ext.cog_slash(
        ...
        description="Allows you to create Private Channels :)",
        options=[
            {
                "name": "create",
    ...
    async def private_channel(self, ctx: SlashContext, **kwargs):
        ...
        guild_data_sql = (
            "select category_id, is_enabled from private_channels_guild_data where guild_id = %s"
        )
        ...
        if "create" in kwargs:
            ...
            if user_has_private_channel:
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "You already have a private channel!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            guild = ctx.guild
            channel_created = await category.create_voice_channel(
                channel_name,
            ...
            await ctx.send(
                embed=utils.return_embed(
                    ctx,
                    "Private Channel",
                    f'Private channel with the name "{channel_name}" got created: {channel_created.mention}',
                    discord.Color.green(),
                ),
                hidden=True,
            )
            ...
            if not user_has_private_channel:
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "You don't have a private channel!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return

            private_channel: discord.VoiceChannel = discord.utils.get(
                ctx.guild.voice_channels, id=user_private_channel_id
            )

            if private_channel is None:
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "Could not find private channel!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            ...
            await ctx.send(
                embed=utils.return_embed(
                    ctx, "Private Channel", "Private channel got deleted!", discord.Color.green()
                ),
                hidden=True,
            )
            ...
            cursor.execute(
                update_sql,
                (str(user_whitelisted_users), ctx.guild.id, private_channel.id, ctx.author.id),
            )
            utils.DB_CONNECTOR.commit()
            ...
            await ctx.send(
                embed=utils.return_embed(
                    ctx,
                    "Private Channel",
                    f"Successfully allowed {member_to_allow.mention} to access the private channel!",
                    discord.Color.green(),
                ),
                hidden=True,
            )
            ...
            await private_channel.set_permissions(
                member_to_disallow, overwrite=discord.PermissionOverwrite(connect=False)
            )

            if member_to_disallow in private_channel.members:
                # user is in private channel, he's getting kicked
                await member_to_disallow.move_to(
                    channel=None, reason="User got denied access in a private channel!"
                )
            ...
            await ctx.send(
                embed=utils.return_embed(
                    ctx,
                    "Private Channel",
                    f"Successfully denied {member_to_disallow.mention} access to the private channel!",
                    discord.Color.green(),
                ),
                hidden=True,
            )
            ...
            if not ctx.author.guild_permissions.administrator:
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "Insufficient permissions!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            ...
            if not isinstance(channel_to_remove, discord.VoiceChannel):
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "You didn't select a voice channel!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            ...
            get_channel_sql = (
                "select * from private_channels where channel_id = %s and guild_id = %s"
            )
            ...
            if result is None:
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "That is not a private channel!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            ...
            if channel_to_remove is None:
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "That private channel was not found!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            ...
            await ctx.send(
                embed=utils.return_embed(
                    ctx,
                    "Private Channel",
                    f"The private channel {'from ' + owner_of_private_channel.mention if owner_of_private_channel is not None else ''} got deleted!",
                    discord.Color.green(),
                ),
                hidden=True,
            )
            ...
            if value == "on":
                ...
                if cursor.fetchone() is None:
                    ...
                    category = (
                        ...
                        else await ctx.guild.create_category("Private-Channels")
                    )
                ...
                await ctx.send(
                    embed=utils.return_embed(
                        ctx,
                        "Private Channels",
                        "Private channels have been enabled!",
                        discord.Color.green(),
                    ),
                    hidden=True,
                )
                ...
                del_sql = (
                    "update private_channels_guild_data set is_enabled = %s where guild_id = %s"
                )
                ...
                await ctx.send(
                    embed=utils.return_embed(
                        ctx,
                        "Private Channels",
                        "Private channels have been disabled!",
                        discord.Color.green(),
                    ),
                    hidden=True,
                )
            ...
            if not isinstance(new_category, discord.CategoryChannel):
                await ctx.send(
                    embed=utils.return_embed(
                        ctx, "Error", "You didn't select a category!", discord.Color.red()
                    ),
                    hidden=True,
                )
                return
            ...
            if cursor.fetchone() is None:
                ...
                update_sql = (
                    "update private_channels_guild_data set category_id = %s where guild_id = %s"
                )
            ...
            await ctx.send(
                embed=utils.return_embed(
                    ctx,
                    "Private Channels",
                    f"All future private channels will now be created in {new_category.mention}!",
                    discord.Color.green(),
                ),
                hidden=True,
            )
